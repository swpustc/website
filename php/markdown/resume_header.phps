...
if (isset($_GET['write'])) {
  $resume_write = $_GET['write'];
} else {
  $resume_write = 'false';
}
if (preg_match('/^(t(rue)?|on|1)$/i', $resume_write)) {
  $resume_lang_write = true;
} else {
  $resume_lang_write = false;
}
...
$file_html = MarkdownExtra::defaultTransform($file_text);
$file_enus_html = $file_enus_exist ? MarkdownExtra::defaultTransform($file_enus_text) : $error_enus;
$file_zhcn_html = $file_zhcn_exist ? MarkdownExtra::defaultTransform($file_zhcn_text) : $error_zhcn;
$file_zhtw_html = $file_zhtw_exist ? MarkdownExtra::defaultTransform($file_zhtw_text) : $error_zhtw;
