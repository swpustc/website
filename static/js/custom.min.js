...
h.data("dir","");if(f==="left"){h.cycle("next")}if(f==="right"){h.data("dir","prev");h.cycle("prev")}}d.swipe({swipeLeft:c,swipeRight:c,allowPageScroll:"auto"})}}})();(function(){if($(".tweet").length){$(".tweet").tweet({username:"fanfbmltemplate",page:1,avatar_size:40,count:4,loading_text:"loading ..."}).bind("loaded",function(){var c=$(this).find(".tweet_list")})}})();(function(){var d=$(".markdown"),e=d.find(".alignright-image");if(e.length){var c=function(){e.each(function(){var g=$(this),f=g.next();if(f){f.width(d.width()-g.outerWidth()-24)}})};c();$(window).load(c).resize(c)}})();(function(){var k=".domain-status-",E=$(k+"cdn"),i=$(k+"freeshell"),o=$(k+"static"),z=$("#map.cdn-map"),q=z.length,H="gmap-setup",r="addMarkers",f="removeAllMarkers",C=function(M,N){return $.Deferred(function(O){if(N&&N!=""){if(M.match("image")!==null){$("<img/>").load(function(){O.resolve()}).error(function(){O.reject()}).attr("src",N)}else{if(M.match("json")!==null){$.getJSON(N,function(P){O.resolve()}).error(function(){O.reject()})}else{$.get(N,function(P){O.resolve()}).error(function(){O.reject()})}}}else{O.reject()}}).promise()},l=function(M){return $.Deferred(function(N){if(M&&M!=""){$.getJSON(M,function(O){var P=0,R=0;for(var Q in O){R++}$.each(O,function(S,T){$.when(C(S,T)).always(function(){if(++P==R){N.resolve()}})})}).error(function(){N.reject()})}else{N.reject()}}).promise()},w=function(N,M){return $.Deferred(function(O){var Q=function(){N.addClass(H);O.resolve()},P={address:"China",onComplete:Q};if(M&&M!=""){$.getJSON(M,function(R){R.onComplete=Q;N.gMap(R)}).error(function(){N.gMap(P)})}else{N.gMap(P)}}).promise()},t=function(N,M){return $.Deferred(function(O){if(M&&M!=""){$.getJSON(M,function(P){N.gMap(f);N.gMap(r,P);O.resolve()}).error(function(){O.reject()})}else{O.reject()}}).promise()},G="json-preload",j="json-center",g="json-marker",n=10000,D=500;if(E.length||i.length||o.length||q){var u="/domain-test.png?t=",h="//freeshell.swpbox.info"+u,s="//static.swpbox.info"+u,p=null,x=null,F="highlight",e=F+"1",m=F+"2",I=F+"3",d=F+"4",v=" ",B=e+v+m+v+I+v+d,y=function(M){return $.Deferred(function(N){if(M&&M!=""){$("<img/>").load(function(){N.resolve()}).error(function(){N.reject()}).attr("src",M+(new Date()).getTime())}else{N.reject()}}).promise()},K="-nl",c="-up",A="-dn",J={a:function(){$.when(y(h)).done(function(){if(p!==true){p=true;i.each(function(){$(this).removeClass(B).addClass(m)});E.each(function(){$(this).removeClass(B).addClass(x===null?d:(x?m:d))});if(q){var M=0;z.each(function(){var O=$(this),N=g+c+(x===null?K:(x?c:A));$.when(t(O,O.attr(N))).fail(function(){p=false}).always(function(){if(++M==q){setTimeout(J.b,D)}})})}else{setTimeout(J.b,D)}}else{setTimeout(J.b,D)}}).fail(function(){if(p!==false){p=false;i.each(function(){$(this).removeClass(B).addClass(I)});E.each(function(){$(this).removeClass(B).addClass(x===null?e:(x?d:I))});if(q){var M=0;z.each(function(){var O=$(this),N=g+A+(x===null?K:(x?c:A));$.when(t(O,O.attr(N))).fail(function(){p=true}).always(function(){if(++M==q){setTimeout(J.b,D)}})})}else{setTimeout(J.b,D)}}else{setTimeout(J.b,D)}})},b:function(){$.when(y(s)).done(function(){if(x!==true){x=true;o.each(function(){$(this).removeClass(B).addClass(m)});E.each(function(){$(this).removeClass(B).addClass(p?m:d)});if(q){var M=0;z.each(function(){var O=$(this),N=g+(p?c:A)+c;$.when(t(O,O.attr(N))).fail(function(){x=false}).always(function(){if(++M==q){setTimeout(J.a,n)}})})}else{setTimeout(J.a,n)}}else{setTimeout(J.a,n)}}).fail(function(){if(x!==false){x=false;o.each(function(){$(this).removeClass(B).addClass(I)});E.each(function(){$(this).removeClass(B).addClass(p?d:I)});if(q){var M=0;z.each(function(){var O=$(this),N=g+(p?c:A)+A;$.when(t(O,O.attr(N))).fail(function(){x=true}).always(function(){if(++M==q){setTimeout(J.a,n)}})})}else{setTimeout(J.a,n)}}else{setTimeout(J.a,n)}})}};if(q){var L=0;z.each(function(){var P=$(this),M=P.attr(G),N=P.attr(j),O=P.attr(g);$.when(l(M)).always(function(){$.when(w(P,N)).always(function(){$.when(t(P,O)).always(function(){if(++L==q){setTimeout(J.a,D)}})})})})}else{setTimeout(J.a,D)}}else{$("#map").each(function(){var P=$(this),M=P.attr(G),N=P.attr(j),O=P.attr(g);$.when(l(M)).always(function(){$.when(w(P,N)).always(function(){t(P,O)})})})}})();(function(){$(".spanwords").each(function(){var d=$(this),c=d.text().replace(/./g,"<span>$&</span>");d.html(c)})})()});(function(){$.fn.jflickrfeed=function(c,e){c=$.extend(true,{flickrbase:"http://api.flickr.com/services/feeds/",feedapi:"photos_public.gne",limit:20,qstrings:{lang:"en-us",format:"json",jsoncallback:"?"},cleanDescription:true,useTemplate:true,itemTemplate:"",itemCallback:function(){}},c);var a=c.flickrbase+c.feedapi+"?";var d=true;for(var b in c.qstrings){if(!d){a+="&"}a+=b+"="+c.qstrings[b];d=false}return $(this).each(function(){var g=$(this);var f=this;$.getJSON(a,function(h){$.each(h.items,function(m,p){if(m<c.limit){if(c.cleanDescription){var o=/<p>(.*?)<\/p>/g;var j=p.description;if(o.test(j)){p.description=j.match(o)[2];
