...
h.data("dir","");if(f==="left"){h.cycle("next")}if(f==="right"){h.data("dir","prev");h.cycle("prev")}}d.swipe({swipeLeft:c,swipeRight:c,allowPageScroll:"auto"})}}})();(function(){if($(".tweet").length){$(".tweet").tweet({username:"fanfbmltemplate",page:1,avatar_size:40,count:4,loading_text:"loading ..."}).bind("loaded",function(){var c=$(this).find(".tweet_list")})}})();(function(){var d=$(".markdown"),e=d.find(".alignright-image");if(e.length){var c=function(){e.each(function(){var g=$(this),f=g.next();if(f){f.width(d.width()-g.outerWidth()-24)}})};c();$(window).load(c).resize(c)}})();(function(){var z=".domain-status-",o=$(z+"cdn"),l=$(z+"freeshell"),s=$(z+"static"),u=$("#map.cdn-map"),g=u.length,K="addMarker",H="removeAllMarkers",C=function(M,L){return $.Deferred(function(N){var O="China";if(L&&L!=""){$.getJSON(L,function(P){M.gMap(P);N.resolve()}).error(function(){M.gMap({address:O});N.resolve()})}else{M.gMap({address:O});N.resolve()}}).promise()},d=function(M,L){return $.Deferred(function(N){if(L&&L!=""){$.getJSON(L,function(O){M.gMap(H);$.each(O,function(P,Q){M.gMap(K,Q)});N.resolve()}).error(function(){N.resolve()})}else{N.resolve()}}).promise()},k="json-center",v="json-marker",J=40000,A=500;if(o.length||l.length||s.length||g){var w="/domain-test.png?t=",c="//freeshell.swpbox.info"+w,E="//static.swpbox.info"+w,y=null,t=null,j="highlight",F=j+"1",r=j+"2",e=j+"3",q=j+"4",f=" ",h=F+f+r+f+e+f+q,B=function(L){return $.Deferred(function(M){$("<img/>").load(function(){M.resolve()}).error(function(){M.reject()}).attr("src",L+(new Date()).getTime())}).promise()},n=u.attr(v+"-hash"),I="/gmap",i=".json"+(n?("?t="+n):""),m="_null",D="_up",x="_down",G={a:function(){$.when(B(c)).done(function(){if(y!==true){y=true;l.each(function(){$(this).removeClass(h).addClass(r)});o.each(function(){$(this).removeClass(h).addClass(t===null?q:(t?r:q))});if(g){var L=0;u.each(function(){var N=$(this),M=I+D+(t===null?m:(t?D:x))+i;$.when(d(N,M)).done(function(){if(++L==g){setTimeout(G.b,A)}})})}else{setTimeout(G.b,A)}}else{setTimeout(G.b,A)}}).fail(function(){if(y!==false){y=false;l.each(function(){$(this).removeClass(h).addClass(e)});o.each(function(){$(this).removeClass(h).addClass(t===null?F:(t?q:e))});if(g){var L=0;u.each(function(){var N=$(this),M=I+x+(t===null?m:(t?D:x))+i;$.when(d(N,M)).done(function(){if(++L==g){setTimeout(G.b,A)}})})}else{setTimeout(G.b,A)}}else{setTimeout(G.b,A)}})},b:function(){$.when(B(E)).done(function(){if(t!==true){t=true;s.each(function(){$(this).removeClass(h).addClass(r)});o.each(function(){$(this).removeClass(h).addClass(y===null?q:(y?r:q))});if(g){var L=0;u.each(function(){var N=$(this),M=I+(y===null?m:(y?D:x))+D+i;$.when(d(N,M)).done(function(){if(++L==g){setTimeout(G.a,J)}})})}else{setTimeout(G.a,J)}}else{setTimeout(G.a,J)}}).fail(function(){if(t!==false){t=false;s.each(function(){$(this).removeClass(h).addClass(e)});o.each(function(){$(this).removeClass(h).addClass(y===null?F:(y?q:e))});if(g){var L=0;u.each(function(){var N=$(this),M=I+(y===null?m:(y?D:x))+x+i;$.when(d(N,M)).done(function(){if(++L==g){setTimeout(G.a,J)}})})}else{setTimeout(G.a,J)}}else{setTimeout(G.a,J)}})}};if(g){var p=0;u.each(function(){var N=$(this),L=N.attr(k),M=N.attr(v);$.when(C(N,L)).done(function(){$.when(d(N,M)).done(function(){if(++p==g){setTimeout(G.a,A+A)}})})})}else{setTimeout(G.a,A+A)}}else{$("#map").each(function(){var N=$(this),L=N.attr(k),M=N.attr(v);$.when(C(N,L)).done(function(){d(N,M)})})}})();(function(){$(".spanwords").each(function(){var d=$(this),c=d.text().replace(/./g,"<span>$&</span>");d.html(c)})})()});(function(){$.fn.jflickrfeed=function(c,e){c=$.extend(true,{flickrbase:"http://api.flickr.com/services/feeds/",feedapi:"photos_public.gne",limit:20,qstrings:{lang:"en-us",format:"json",jsoncallback:"?"},cleanDescription:true,useTemplate:true,itemTemplate:"",itemCallback:function(){}},c);var a=c.flickrbase+c.feedapi+"?";var d=true;for(var b in c.qstrings){if(!d){a+="&"}a+=b+"="+c.qstrings[b];d=false}return $(this).each(function(){var g=$(this);var f=this;$.getJSON(a,function(h){$.each(h.items,function(m,p){if(m<c.limit){if(c.cleanDescription){var o=/<p>(.*?)<\/p>/g;var j=p.description;if(o.test(j)){p.description=j.match(o)[2];if(p.description!=undefined){p.description=p.description.replace("<p>","").replace("</p>","")}}}p["image_s"]=p.media.m.replace("_m","_s");p["image_t"]=p.media.m.replace("_m","_t");p["image_m"]=p.media.m.replace("_m","_m");p["image"]=p.media.m.replace("_m","");p["image_b"]=p.media.m.replace("_m","_b");delete p.media;if(c.useTemplate){var n=c.itemTemplate;for(var l in p){var k=new RegExp("{{"+l+"}}","g");n=n.replace(k,p[l])}g.append(n)}c.itemCallback.call(f,p)}});if($.isFunction(e)){e.call(f,h)}})})}})();jQuery.cookie=function(b,j,m){if(typeof j!="undefined"){m=m||{};if(j===null){j="";m.expires=-1}var e,l,g,a;if(typeof m.expires!="undefined"&&m.expires===null){e=""}else{if(m.expires&&(typeof m.expires=="number"||m.expires.toUTCString)){var f;if(typeof m.expires=="number"){f=new Date();
